...
subroutine dotgen_merkle(this, unit_)
   	class(merkle), intent(inout) :: this
    	integer, intent(in) :: unit_
	write(unit_, '(A)') 'graph{'
	call this%dotgenrec_merkle(this%tophash, unit_)
	write(unit_, '(A)') '}'
end subroutine dotgen_merkle

subroutine dotgenrec_merkle(this, tmp, unit_)
   	class(merkle), intent(inout) :: this
	...
	write (unit_, '(A,I5,A,A,A)') ' ', tmp%uid, ' [label="', tmp%hash, '"shape=rect style=filled fillcolor=lightgreen];'
	...
	call this%dotgenrec_merkle(tmp%left, unit_)
    	call this%dotgenrec_merkle(tmp%right, unit_)
	if (associated(tmp%dataref)) then
		write (unit_, '(A,I5,A,A,A)') ' ', tmp%dataref%uid, ' [label="', tmp%dataref%value, '" shape=rect style=filled fillcolor=orange];'
